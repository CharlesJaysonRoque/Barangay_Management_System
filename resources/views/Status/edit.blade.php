...
<div class="edit-container">
    <div class="edit-card">
        <div class="edit-header">
            ...
            <div>
                <h1 class="edit-title">Edit Status</h1>
                <p class="edit-subtitle">Update status description</p>
            </div>
        </div>
        ...
        <form action="{{ route('statuses.update', $status->id) }}" method="POST" class="edit-form">
            ...
            @if($totalUsage > 0)
                <div class="warning-box">
                    ...
                    <div>
                        <strong>Warning:</strong> This status is currently in use.
                        @if($usedInComplaints > 0) · {{ $usedInComplaints }} complaint(s) @endif
                        @if($usedInProjects > 0) · {{ $usedInProjects }} project(s) @endif
                        <br>Updating will affect all associated records.
                    </div>
                </div>
            @endif
            ...
            <div class="info-box">
                ...
                <div class="info-content">
                    <div class="info-row">
                        ...
                        <span class="info-value">{{ $status->created_at ? $status->created_at->format('F d, Y h:i A') : 'N/A' }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Last updated:</span>
                        <span class="info-value">{{ $status->updated_at ? $status->updated_at->diffForHumans() : 'N/A' }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Status ID:</span>
                        <span class="info-value">#{{ $status->id }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Associated items:</span>
                        <span class="info-value">{{ $totalUsage }} total ({{ $usedInComplaints }} complaints, {{ $usedInProjects }} projects)</span>
                    </div>
                </div>
            </div>
